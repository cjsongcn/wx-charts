...
function getYAxisTextList(series, opts, config) {
  ...
  if (!data.length) return []; // 修复data为空导致minData 和maxData 为 Infinity引起程序卡死
...
var Charts = function Charts(opts, _component) {
  ...
  this.query = _component ? _component.createSelectorQuery() : wx.createSelectorQuery();
  // store calcuated chart data
  // such as chart point coordinate

  this.query.select(opts.canvasId).fields({ node: true, size: true }).exec(function (res) {
    ...
    drawCharts.call(_this, opts.type, opts, config$$1, ctx);
  });
};
...
Charts.prototype.updateData = function () {
  ...
  this.query.select(this.opts.canvasId).fields({ node: true, size: true }).exec(function (res) {
    var canvas = res[0].node;
    var ctx = canvas.getContext("2d");
    ...
    drawCharts.call(_this2, _this2.opts.type, _this2.opts, _this2.config, ctx);
  });
};
