...
export function drawColumnDataPoints(
  ...
  let { xAxisPoints, eachSpacing } = getXAxisPoints(
    opts.categories,
    opts,
    config
  );
...
export function drawXAxis(categories, opts, config, context) {
  let { xAxisPoints, startX, endX, eachSpacing } = getXAxisPoints(
    categories,
    opts,
    config
  );
  ...
  let validWidth =
    ...
    2 * config.padding -
    config.yAxisWidth -
    config.yAxisTitleWidth;
  let maxXAxisListLength = Math.min(
    categories.length,
  ...
  if (config._xAxisTextAngle_ === 0) {
    ...
    categories.forEach(function (item, index) {
      ...
      let { transX, transY } = calRotateTranslate(
...
export function drawRadarDataPoints(
  ...
  let radarDataPoints = getRadarDataPoints(
    coordinateAngle,
    centerPosition,
    radius,
    series,
